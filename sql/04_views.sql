...
-- VIEW: vw_order_360
-- OBJETIVO:
-- Consolidar informações de pedidos, lojas,
-- hubs, canais, entregas, motoristas e pagamentos
-- em uma visão analítica única.
--
-- GRANULARIDADE:
-- 1 linha por pedido
--
...
--
-- USO:
-- Base principal para dashboards analíticos
-- e indicadores executivos no Power BI.
-- =====================================================
...
LEFT JOIN dw.dim_store s
  ON s.store_id = fo.store_id

LEFT JOIN dw.dim_hub h
  ON h.hub_id = s.hub_id

LEFT JOIN dw.dim_channel c
  ON c.channel_id = fo.channel_id

LEFT JOIN dw.fact_deliveries d
  ON d.delivery_order_id = fo.delivery_order_id

LEFT JOIN dw.dim_driver dr
  ON dr.driver_id = d.driver_id

LEFT JOIN dw.fact_payments p
  ON p.payment_order_id = fo.payment_order_id;


-- =====================================================
-- VIEW: vw_kpi_daily
-- OBJETIVO:
-- Consolidar KPIs diários operacionais
-- e financeiros para análise temporal.
--
-- GRANULARIDADE:
-- 1 linha por dia
--
...
--
-- USO:
-- Dashboard executivo e análise de tendência
-- operacional ao longo do tempo.
-- =====================================================

DROP VIEW IF EXISTS analytics.vw_kpi_daily;
...
SELECT
  ...
  SUM(
    CASE
      WHEN order_status ILIKE '%cancel%' THEN 1
      ELSE 0
    END
  ) AS canceled_orders,
...
-- =====================================================
-- VIEW: vw_kpi_by_state
-- OBJETIVO:
-- Consolidar indicadores operacionais
-- e financeiros por estado.
--
-- GRANULARIDADE:
-- 1 linha por estado
--
...
--
-- USO:
-- Comparação regional de performance
...
-- =====================================================
-- VIEW: vw_kpi_by_channel
-- OBJETIVO:
-- Consolidar indicadores financeiros
-- e operacionais por canal de venda.
--
-- GRANULARIDADE:
-- 1 linha por canal
--
